...
def parse_pdf_directly(pdf_filepath):
    ...
    def is_valid_title(t):
        ...
        if any(bad in t_low for bad in bad_words):
            return False
        return True

    # 2. Robust Real Project Title Extraction (Filters out website URLs/domains)
    project_name = None
    
    # Strategy A: Parse Prominent Title Line from Page 1 and Page 2 Text
    ...
    # Strategy B: Check PDF Document Metadata Title (if valid and not a URL)
    if not project_name and doc.metadata and doc.metadata.get("title"):
